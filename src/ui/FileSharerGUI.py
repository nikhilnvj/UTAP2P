...
class FileSharerGUI(QMainWindow,Ui_MainWindow):
    ...
    def __init__(self,initialPeer,ttl=2,maxKnownPeers=5,port=1234,parent=None):
        QtGui.QWidget.__init__(self,parent)
        self.ui=Ui_MainWindow()
        self.ui.setupUi(self)
        hostName,port=initialPeer.split(':')
        self.peerHandler=P2PMessageHandler(maxKnownPeers,port)
        t=Thread(target=self.peerHandler.listenForConnections(),args=[])
        t.start()
        self.peerHandler.buildPeerList(hostName, int(port), ttl)
        self.updatePeers()
...
if __name__=="__main__":
    ...
    myapp=FileSharerGUI("localhost:1234")
